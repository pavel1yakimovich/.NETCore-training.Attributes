...
namespace DoSomethingClient
{
    public class DomainAssemblyLoader : MarshalByRefObject
    {
        ...
        public Result LoadFile(string path, Input data)
        {
            ...
            var typeNotIDoSmth = types.Where(t => t.GetInterface("IDoSomething") == null && Attribute.IsDefined(t, typeof(DoSomethingAttribute))).FirstOrDefault(); // TODO: Find first type that has DoSomething attribute and don't implement IDoSomething.
            MethodInfo mi = typeNotIDoSmth.GetMethod("DoSomething"); // TODO: MethodInfo mi = type.GetMethod("DoSomething");
            var instance = Activator.CreateInstance(typeNotIDoSmth);

            Result result = mi.Invoke(instance, new object[1] { data }) as Result; // TODO: result = mi.Invoke();
